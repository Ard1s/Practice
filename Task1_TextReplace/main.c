        ...
        case WM_COMMAND:
            ...
            switch (LOWORD (wParam)) {
                ...
                case IDM_LIGTHSTYLE:
                    if (glCurrentBitmap) 
                    {
                        newBitmap = GetLightStyleBitmap(glCurrentBitmap);
                        UpdateCurrentBitmap(newBitmap, hwnd);
                    }
                    break;
                case IDM_DARKSTYLE:
                    if (glCurrentBitmap) 
                    {
                        newBitmap = GetDarkStyleBitmap(glCurrentBitmap);
                        UpdateCurrentBitmap(newBitmap, hwnd);
                    }
                    break;
                case IDM_HORIZONSTYLE:
                    if (glCurrentBitmap) 
                    {
                        newBitmap = GetHorizonStyleBitmap(glCurrentBitmap);
                        UpdateCurrentBitmap(newBitmap, hwnd);
                    }
                case IDM_BIGSTYLE:
                    if (glCurrentBitmap)
                    {
                        newBitmap = GetBigStyleBitmap(glCurrentBitmap);
                        UpdateCurrentBitmap(newBitmap, hwnd);
                    }
                    break;
